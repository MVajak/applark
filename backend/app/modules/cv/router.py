...
async def get_cv_documents(
    ...
    async with SessionLocal() as session:
        documents = await cv_service.list_documents(
            session, current_user.id, kind=kind, limit=limit, offset=offset
        )
...
async def get_cv_document(
    ...
    async with SessionLocal() as session:
        document = await cv_service.get_document_with_chunks(session, current_user.id, document_id)
        if document is None:
...
async def delete_cv_document(
    ...
    async with SessionLocal() as session:
        deleted = await cv_service.delete_document(session, current_user.id, document_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="CV document not found")
        await session.commit()
...
async def cv_events(
    ...
    """SSE stream of CvProcessingEvent JSON frames. Auth via ``?token=``."""
